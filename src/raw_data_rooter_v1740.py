...
class RawDataRooter():
    ...
    def sanity_check(self):
        ...
        raw_data_file = RawDataFile(self.args.if_path, n_boards=N_BOARDS, ETTT_flag=ETTT_FLAG, DAQ_Software=DAQ_SOFTWARE)
    ...
    def preview_file(self):
        ...
        if len(n_samples_v1730) != 1:
            sys.exit("Something wrong. All channels of any V1730 triggers must have the same n_samples.")
        elif len(n_samples_v1740) != 1:
            sys.exit("Something wrong. All channels of any V1740 triggers must have the same n_samples.")
        else:
            self.n_samples_v1730 = n_samples_v1730.pop()
            self.n_samples_v1740 = n_samples_v1740.pop()
    ...
    def create_output_file(self):
        ...
        if f_ext=='.root':
            ...
            ch_var_v1730 = zeros([DUMP_SIZE//N_BOARDS, self.n_samples_v1730], dtype=uint16) # dummy var, data structure
            ch_var_v1740 = zeros([DUMP_SIZE//N_BOARDS, self.n_samples_v1740], dtype=uint16) # dummy var, data structure
            vars_type = {}
            for ch in ch_names:
